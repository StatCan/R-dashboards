...
ev_plot_data <- ev_nmvr_data %>% 
  ...
  mutate(geo = ifelse(geo %in% other_provinces, 'Other provinces', geo)) %>% 
  group_by(geo) %>% 
...
ev_plot_data %>% 
  arrange(desc(geo)) %>% 
  mutate(prop = amount / sum(ev_plot_data$amount) * 100) %>% 
...
ev_nmvr_data %>% 
  ...
  ungroup() %>% 
  group_by(geo) %>% 
  mutate(approx_total = cumsum(amount)) %>% 
  filter(year == max_year) %>% 
  select(-year) %>% 
  adorn_totals('row', name = 'Canada') %>% 
  rename('Province/Territory' = geo, !!paste(max_year) := amount, 'Approx. EV Total' = approx_total) %>% 
  kable()
```
